%Improved computation of population, hw3 p2
%Computes and graphs the populations of two species under competition and 
%accounts for an additional factor on the population of species X
%Duncan Di Mauro
%805163177
...
epsilon = 0.3;
delta = 0.2;
sigma = -0.4; %New variable for new factor
...
%This variable is the time when the new factor begins to change alpha
t_alphafactorstart = 5;   
...
for k = 0:t_steps
    ...
    %New factor that starts at t = 5
    
    if t >= t_alphafactorstart
        alpha = alpha*(exp((-1*(t-t_alphafactorstart)^2)/(2*sigma^2))); 
    end
